...
class DicePoolReporter:
    ...
    def __init__(self, dice_pool_list):
    ...
    def round_to_percent(self, count, significant_digits):
        return round((count / self.total_results * 100), significant_digits)

    def report_result_discrete(self, result_counts):
        for results, count in sorted(result_counts.items(), reverse=True):
            if results >= 0:
                print('', results, ':', self.round_to_percent(count, 4), '%')
            else:
                print(results, ':', self.round_to_percent(count, 4), '%')
